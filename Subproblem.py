...
class SubProblem:
    ...
    def create_model(self):
        ...
        self.model.addConstr((gp.quicksum(
            self.m_f[f] * self.x[f] + self.m_g * self.x[g] for f in range(self.F) for g in range(self.G)) <= self.M))
        # 电池容量约束
        for v in self.V:
            self.model.addConstr(gp.quicksum(self.b[v, f] * self.x[f] for f in self.F) + gp.quicksum(
                self.b[v, g] * self.x[g] for g in self.G) == 0)
        # 节点流平衡约束
        for s in self.S:
